...
mod tests {
    ...
    use crate::configuration::{
        LlmProviderType, RetryPolicy, RetryStrategy, StatusCodeConfig, StatusCodeEntry,
        TimeoutRetryConfig, RetryAfterHandlingConfig, BlockScope, ApplyTo, HighLatencyConfig, LatencyMeasure,
    };
    use bytes::Bytes;
    ...
    async fn test_max_retry_duration_ms_exceeded_mid_retry_stops_with_most_recent_error() {
        ...
        assert!(!err.attempts.is_empty(), "Should have at least one attempt recorded");
    ...
    async fn test_max_retry_duration_timer_starts_on_first_retry_not_original_request() {
        ...
        assert!(ctx.retry_start_time.is_none(), "retry_start_time should be None before execute");
    ...
    async fn test_empty_fallback_models_different_provider_uses_provider_list() {
        ...
        assert!(result.is_ok(), "Should succeed after falling back to Provider_List");
    ...
    async fn test_timeout_triggers_retry_to_different_provider() {
        ...
        assert!(result.is_ok(), "Should succeed after timeout retry to different provider");
        let calls = call_log.lock().unwrap();
    ...
    async fn test_timeout_without_on_timeout_uses_defaults() {
        ...
        assert!(result.is_ok(), "Should succeed after timeout retry using defaults");
        let calls = call_log.lock().unwrap();
        assert_eq!(calls[0], "openai/gpt-4o");
        assert_eq!(calls[1], "anthropic/claude-3-5-sonnet");
    }
    ...
    async fn test_timeout_error_records_duration_in_attempt() {
        ...
        let result = orchestrator
            .execute(
                ...
                &mut ctx,
                |_body, _provider| async {
                    Err(TimeoutError { duration_ms: 12345 })
                },
            )
            .await;
        ...
        let _result = orchestrator
            .execute(
                ...
                |_body, _provider| async {
                    ...
                    Ok(make_response_with_headers(429, vec![("retry-after", "not-a-number")]))
                },
            )
            .await;
    ...
    async fn test_high_latency_completed_response_delivered_and_block_state_created() {
        ...
        assert!(result.is_ok(), "Completed-but-slow response should be delivered to client");
    ...
    async fn test_high_latency_completed_response_request_scoped_block() {
        ...
        assert!(
            ctx.request_latency_block_state.contains_key("openai/gpt-4o"),
    ...
    async fn test_timeout_dual_classification_records_high_latency_event() {
        ...
        assert!(
            ctx.errors.iter().any(|e| matches!(e.error_type, AttemptErrorType::Timeout { .. })),
    ...
    async fn test_successful_request_below_threshold_does_not_remove_latency_block() {
        ...
        assert!(result.is_ok(), "Request should succeed via secondary provider");
