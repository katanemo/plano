        ...
        match strategy {
            RetryStrategy::SameModel => {
                ...
                let candidate = all_providers.iter().find(|p| {
                    p.model.as_deref() == Some(primary_model) && !attempted.contains(primary_model)
                });
            ...
            RetryStrategy::SameProvider | RetryStrategy::DifferentProvider => {
                        ...
                        if is_ra_blocked(fallback_model) {
                            ...
                            if let Some(ref ra_config) = primary_ra_config {
                                if let Some(remaining) = self.check_ra_remaining_duration(
                                    ...
                                    shortest_remaining =
                                        Some(shortest_remaining.map_or(secs, |s: u64| s.min(secs)));
                                }
                            }
                            continue;
                        }
                ...
                // Phase 2: Fall back to Provider_List, collecting eligible
                // candidates and selecting one at random (as requested by
                // maintainer review) instead of deterministic first-match.
                let mut candidates: Vec<usize> = Vec::new();
                for (idx, p) in all_providers.iter().enumerate() {
                ...
                if !candidates.is_empty() {
                    ...
                    let chosen = &all_providers[chosen_idx];
                    info!(
                        "Provider_List: randomly selected {} from {} eligible candidates (request_id={})",
                        chosen.model.as_deref().unwrap_or("unknown"),
                        candidates.len(),
                        request_context.request_id
                    );
                    return Ok(ProviderSelectionResult::Selected(chosen));
                }
...
mod tests {
    ...
    #[test]
    fn randomly_selects_from_eligible_provider_list_candidates() {
        let providers = vec![
        ...
        // different_provider from openai should randomly pick one of the anthropic models
        let result = selector.select(
            RetryStrategy::DifferentProvider,
            "openai/gpt-4o",
            &[],
        ...
        match result.unwrap() {
            ProviderSelectionResult::Selected(p) => {
                let model = p.model.as_deref().unwrap();
                assert!(
                    model == "anthropic/claude-3-opus" || model == "anthropic/claude-3-5-sonnet",
                    "expected an anthropic model, got {}",
                    model
                );
            }
            _ => panic!("expected Selected"),
    ...
    fn empty_fallback_models_uses_provider_list() {
        ...
        // Should randomly pick one of the eligible different-provider candidates
        match result.unwrap() {
            ProviderSelectionResult::Selected(p) => {
                let model = p.model.as_deref().unwrap();
                assert!(
                    model == "anthropic/claude-3-5-sonnet" || model == "azure/gpt-4o",
                    "expected anthropic or azure model, got {}",
                    model
                );
            }
            _ => panic!("expected Selected"),
    ...
    use crate::configuration::{
        HighLatencyConfig, LatencyMeasure, RetryAfterHandlingConfig, RetryPolicy,
    };

    fn make_provider_with_retry_policy(
        model: &str,
    ...
    fn same_model_global_ra_block_returns_wait_and_retry() {
        ...
        match result.unwrap() {
            ProviderSelectionResult::WaitAndRetrySameModel { wait_duration } => {
                ...
                assert!(
                    wait_duration.as_secs() > 0,
                    "wait_duration should be positive"
                );
                assert!(
                    wait_duration.as_secs() <= 60,
                    "wait_duration should be <= 60s"
                );
            }
    ...
    fn same_model_no_ra_block_returns_selected() {
        ...
        let providers = vec![make_provider_with_retry_policy("openai/gpt-4o", None)];
        let attempted = HashSet::new();
        let ctx = stub_context();
        let selector = ProviderSelector;
        let ra_state = RetryAfterStateManager::new();
    ...
    fn same_model_request_scoped_ra_block_returns_wait_and_retry() {
        ...
        let providers = vec![make_provider_with_retry_policy(
            "openai/gpt-4o",
            Some(ra_config),
        )];
        let attempted = HashSet::new();
    ...
    fn all_candidates_ra_blocked_returns_error_with_shortest_remaining() {
        ...
        match result {
            Err(e) => {
                ...
                assert!(
                    shortest <= 30,
                    "shortest remaining should be <= 30s, got {}",
                    shortest
                );
            }
    ...
    fn same_model_request_scoped_lb_block_returns_error() {
        ...
        assert!(
            result.is_err(),
            "request-scoped LB block should return error for same_model"
        );
    }
    ...
    proptest! {
        ...
        fn prop_fallback_models_priority_ordering(
            ...
            // Collect all eligible Provider_List candidates (not just first).
            let eligible_provider_list: Vec<&str> = all_providers
                .iter()
                .filter_map(|p| {
                    if let Some(ref m) = p.model {
            ...
            match result {
                Ok(ProviderSelectionResult::Selected(p)) => {
                    ...
                    if let Some(expected_fallback) = first_eligible_fallback {
                        ...
                        // Should be one of the eligible Provider_List candidates
                        // (random selection, not necessarily the first).
                        prop_assert!(
                            eligible_provider_list.contains(&selected),
                            "Selected '{}' not in eligible Provider_List candidates {:?}",
                            selected, eligible_provider_list
                        );
                    }
                }
                Ok(ProviderSelectionResult::WaitAndRetrySameModel { .. }) => {
